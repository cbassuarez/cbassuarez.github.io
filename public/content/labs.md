# Labs

Live and experimental routes.

- [(to)complete](/labs/corpus) — one shared sentence, mutated by visitation.
- [String](/labs/string) — shared live string instrument (multi-user lanes, plucks, and resonance).
- [Seb's REPL](/labs/repl) — live-coding score-grid REPL playing through the cbassuarez voices and sample bank.
- [Works list](/labs/works-list) — curated three-work Praetorius page.
- [Chunk Surfer](/labs/chunk-surfer) — navigable chunk-field lab surface.
- [This Person](/labs/this-person) — advertising-profile extraction and return loop.
- [Tell Me About Your Day](/labs/tell-me-about-your-day) — TMAYD submission and reels lab route.
- [Seb Feed](/labs/feed) — live operator activity timeline.
- [Guestbook](/labs/guestbook) — signed public guestbook stream.
- [microwear](/labs/morts) — a one-minute benchmark that bills you, in morts, for the wear it spent.
