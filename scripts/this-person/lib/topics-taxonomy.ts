// this person — Chrome Topics API taxonomy lookup.
//
// `document.browsingTopics()` returns integer topic IDs, taxonomy version, and
// model version. Chrome ships the canonical mapping (path strings) inside the
// browser itself; web pages must look it up. This file is the v2 taxonomy
// published by Chrome (Topics API stable rollout, ~349 entries), inlined so the
// lab can resolve a topic ID without a network call.
//
// We keep the full slash-delimited paths so the wall entry can choose to render
// either the leaf ("Comics") or the path ("Arts & Entertainment / Comics").
//
// Updating: when Chrome ships a new taxonomy version, replace this array. The
// `version` field on each Topics result tells you which one Chrome used.

export const TOPICS_TAXONOMY_V2: ReadonlyArray<string> = [
  "", // index 0 is unused — Topics IDs start at 1.
  "/Arts & Entertainment",
  "/Arts & Entertainment/Acting & Theater",
  "/Arts & Entertainment/Anime & Manga",
  ...
  "/Arts & Entertainment/Music & Audio/Reggae & Caribbean Music",
  ...
  "/Books & Literature/Comics & Graphic Novels",
  ...
  "/Computers & Electronics/Computer Peripherals/Drives & Storage",
  ...
  "/Computers & Electronics/Software/Desktop Publishing/Fonts & Typography",
  ...
  "/Food & Drink/Cooking & Recipes/Cuisines/Vegan Cuisine",
  ...
  "/Games/Billiards",
  "/Games/Card Games",
  "/Games/Chess & Abstract Strategy Games",
  ...
  "/Games/Computer & Video Games/Vehicle Simulators",
  ...
  "/News/Business News/Mergers & Acquisitions",
  "/News/Politics",
  "/News/Sports News",
  "/News/Weather",
  "/News/World News",
  ...
  "/Travel & Transportation/Beaches & Islands",
  "/Travel & Transportation/Bus & Rail",
  ...
  "/Travel & Transportation/Vacation Rentals & Short-Term Stays",
];

// Returns the full slash-delimited taxonomy path for a topic ID, or an empty
// string if the ID is out of range.
export function topicPath(topicId: number): string {
  if (!Number.isInteger(topicId) || topicId <= 0) return "";
  if (topicId >= TOPICS_TAXONOMY_V2.length) return "";
  return TOPICS_TAXONOMY_V2[topicId] || "";
}

// Returns the leaf segment of a topic path. "/Arts & Entertainment/Comics"
// becomes "Comics"; the root "/Arts & Entertainment" stays "Arts & Entertainment".
export function topicLeaf(topicId: number): string {
...
// Returns a short human label suitable for the wall: the leaf segment, or the
// stringified ID prefixed with "Topic" for unknown IDs.
export function topicLabel(topicId: number): string {
  const leaf = topicLeaf(topicId);
  if (leaf) return leaf;
