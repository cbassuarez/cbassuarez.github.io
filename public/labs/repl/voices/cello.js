// Iowa Cello voice — first-class sampled string instrument for seb's REPL.
// Uses a generated local manifest at public/instruments/iowa-cello/manifest.full.json.
// Iowa MIS Cello ships only ff samples (no pp/mf layers), but four strings
// (sul C/G/D/A) and two articulations (arco, pizz). This voice synthesizes pp/mf
...
(function (root) {
  ...
  function normalizeString(value) {
    ...
    if (s === 'C' || s === 'G' || s === 'D' || s === 'A') return s;
    if (s === 'SULC' || s === 'SUL C' || s === 'SUL-C') return 'C';
    if (s === 'SULG' || s === 'SUL G' || s === 'SUL-G') return 'G';
    if (s === 'SULD' || s === 'SUL D' || s === 'SUL-D') return 'D';
    if (s === 'SULA' || s === 'SUL A' || s === 'SUL-A') return 'A';
    return null;
  }

  function normalizeManifest(data) {
    const samples = data && data.samples && typeof data.samples === 'object' ? data.samples : {};
    const normalized = { arco: { C: [], G: [], D: [], A: [] }, pizz: { C: [], G: [], D: [], A: [] } };
    ...
    return {
      id: String((data && data.id) || 'iowa-cello'),
      name: String((data && data.name) || 'Iowa Cello'),
      source: String((data && data.source) || 'University of Iowa Musical Instrument Samples'),
      instrument: String((data && data.instrument) || 'Solo cello'),
      ...
      openStrings: data && data.openStrings ? data.openStrings : { C: 36, G: 43, D: 50, A: 57 },
  ...
  function loadManifest(url) {
    ...
    _manifestPromise = fetch(targetUrl, { credentials: 'omit' })
      .then((r) => {
        if (!r.ok) throw new Error('cello manifest http ' + r.status);
        return r.json();
      })
      .then((data) => {
      ...
      .catch((err) => {
        ...
        console.warn('[repl] cello manifest load failed:', err);
        return _manifest;
      });
  ...
  function loadBuffer(audioCtx, entry) {
    ...
    const promise = fetch(url, { credentials: 'omit' })
      .then((r) => { if (!r.ok) throw new Error('cello sample http ' + r.status); return r.arrayBuffer(); })
      ...
      .catch((err) => {
        _pendingBuffers.delete(key);
        console.warn('[repl] cello sample load failed:', entry.sourceFile || entry.url, err);
        return null;
      });
  ...
  function applyDynamicsAndTone(audioCtx, signal, opts) {
    ...
    lowpass.frequency.value = clamp(950 + Math.pow(force, 0.85) * 9800, 750, 13000);
    lowpass.Q.value = 0.5;
  ...
  // -------------------- core scheduler --------------------
    ...
    function scheduleNote(opts, plan) {
      ...
      src.onended = () => unregister(active);
        ...
        unregister(active);
        console.warn('[repl] cello start failed:', err);
        return false;
      }
    }
  ...
  function prewarm(audioCtx, notes) {
    ...
    return ready().then(() => {
      ...
      const wants = Array.isArray(notes) && notes.length ? notes : ['C2', 'G2', 'D3', 'A3', 'C3'];
      const loads = [];
    ...
    function playCelloChord(opts) {
      ...
      const chordId = `cello-chord-${++_chordCounter}`;
    ...
    function extendCelloChord(opts) {
      const audioCtx = opts && opts.audioCtx;
      if (!audioCtx) return false;
